...
/**
 * @file
 * Contains \Drupal\address_entry\Plugin\Search\AddressEntrySearch.
 */

namespace Drupal\address_entry\Plugin\Search;
...
/**
 * Handles searching for address_term entities.
 *
 ...
 *   address_entry/Plugin/Search.
 * # Clear caches (drush cr).
 * # Add a "Address Book Search" search page at
 ...
 *   address_term items (the number of "contact"
 ...
 *
 * @SearchPlugin(
 *   id = "address_entry_search",
 *   title = @Translation("Address Entry")
 * )
 */
class AddressEntrySearch extends ConfigurableSearchPluginBase implements AccessibleInterface, SearchIndexingInterface {
     ...
     *
     * Note: In our case joins aren't needed because the {address_term} table is
    ...
    /**
     * Constructs \Drupal\address_entry\Plugin\Search\AddressEntrySearch.
     *
    ...
    public function __construct(array $configuration, $plugin_id, $plugin_definition, Connection $database, EntityManagerInterface $entity_manager, ModuleHandlerInterface $module_handler, Config $search_settings, LanguageManagerInterface $language_manager, RendererInterface $renderer, AccountInterface $account = NULL) {
        ...
        $this->addCacheTags(['address_list']);
    }
    ...
    protected function findResults() {
        ...
        // Join on {address_term} and {users_field_data} to get user's published status.
        // This join on {address_term} also serves the advanced search items
        // (in $this->advanced) that are in {address_term}.
        $query->join('address_term', 'c', 'c.id = i.sid');
        ...
        if (!empty($parameters['f']) && is_array($parameters['f'])) {
            ...
            // separated by ':' like 'email:traoantam@gmail.com'.
    ...
    protected function prepareResults(StatementInterface $found) {
        ...
        $entity_storage = $this->entityManager->getStorage('address_term');
        $entity_render = $this->entityManager->getViewBuilder('address_term');
        $keys = $this->keywords;

        foreach ($found as $item) {
            // Render the contact.
            /** @var \Drupal\address_entry\AddressEntryInterface $entity */
            ...
            $rendered .= ' ' . $this->moduleHandler->invokeAll('address_update_index', [$entity]);

            $extra = $this->moduleHandler->invokeAll('address_search_result', [$entity]);
    ...
    public function updateIndex() {
        ...
        $result = $this->database->queryRange("SELECT c.id, MAX(sd.reindex) FROM {address_term} c LEFT JOIN {search_dataset} sd ON sd.sid = c.id AND sd.type = :type WHERE sd.sid IS NULL OR sd.reindex <> 0 GROUP BY c.id ORDER BY MAX(sd.reindex) is null DESC, MAX(sd.reindex) ASC, c.id ASC", 0, $limit, array(':type' => $this->getPluginId()), array('target' => 'replica'));
     ...
     *
     * @param \Drupal\address_entry\AddressEntryInterface $entity
     *   The contact to index.
     */
    protected function indexContact(AddressEntryInterface $entity) {
        $languages = $entity->getTranslationLanguages();
        // 'content_entity_example_contact' comes from the entity type id declared
        // in the annotation for \Drupal\address_entry\Entity\Term.
        // Replace this with your entity's type id.
        $entity_render = $this->entityManager->getViewBuilder('address_term');
        ...
        foreach ($languages as $language) {
            ...
            $extra = $this->moduleHandler->invokeAll('address_update_index', [$entity]);
            foreach ($extra as $t) {
                $text .= $t;
            }
    ...
    public function indexStatus() {
        ...
        $total = $this->database->query('SELECT COUNT(*) FROM {address_term}')->fetchField();
        $remaining = $this->database->query("SELECT COUNT(DISTINCT c.id) FROM {address_term} c LEFT JOIN {search_dataset} sd ON sd.sid = c.id AND sd.type = :type WHERE sd.sid IS NULL OR sd.reindex <> 0", array(':type' => $this->getPluginId()))->fetchField();
    ...
    public function searchFormAlter(array &$form, FormStateInterface $form_state) {
        ...
        $form['advanced']['misc-fieldset']['email'] = array(
            '#type' => 'textfield',
            '#title' => t('Email'),
            '#description' => t('Search %field field for exact matches.', array('%field' => 'Email')),
            '#default_value' => isset($defaults['email']) ? $defaults['email'] : array(),
        );

        $form['advanced']['misc-fieldset']['phone'] = array(
            '#type' => 'textfield',
            '#title' => t('Phone'),
            '#description' => t('Search %field field for exact matches.', array('%field' => 'Phone')),
            '#default_value' => isset($defaults['phone']) ? $defaults['phone'] : array(),
        );
    ...
    public function buildSearchUrlQuery(FormStateInterface $form_state) {
        ...
        if ($form_state->hasValue('email') && !empty(($value = trim($form_state->getValue('email'))))) {
            $filters[] = 'email:' . $value;
            $advanced = TRUE;
        }

        if ($form_state->hasValue('phone') && !empty(($value = trim($form_state->getValue('phone'))))) {
            $filters[] = 'phone:' . $value;
            $advanced = TRUE;
        }
